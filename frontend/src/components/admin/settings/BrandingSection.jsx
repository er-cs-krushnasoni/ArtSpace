...
export default function BrandingSection({ initialData, onSaved }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      <h2 className="text-base font-semibold text-gray-900 mb-1">Shop Logo</h2>
      <p className="text-xs text-gray-400 mb-5">
        Used in your public shop header, emails, and as the admin app icon. JPEG, PNG, or WebP. Max 2MB.
      </p>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-gray-200 text-gray-600 hover:border-violet-400 hover:text-violet-600 transition-all disabled:opacity-60"
          >
            <Upload className="w-4 h-4" />
            {isUploading ? 'Uploading…' : preview ? 'Replace Logo' : 'Upload Logo'}
          </button>
          <p className="text-xs text-gray-400 mt-1.5">Click the image or button to upload</p>
        </div>
      </div>
