...
const getAnalytics = async (req, res) => {
  ...
  const conversionRate = totalQueries > 0
    ...
    : 0;
  // ── Revenue (filter by payment entry date) ────────────────────────────────
  // Fetch tasks that have at least one payment entry in the range.
  // For 'all', reuse taskSnaps (no date filter).
  const revenueSnaps = range === 'all'
    ? taskSnaps
    : await AnalyticsSnapshot.find({
        tenantId: tenantObjId,
        type: 'task',
        isExcluded: { $ne: true },
        ...(dateFilter && { 'paymentEntries.date': dateFilter }),
      }).lean();
  let totalRevenue = 0;
  const revenueByDayMap = {};
  revenueSnaps.forEach((snap) => {
  ...
  // ── Payment breakdown (ALL task snaps ever, no date filter) ───────────────
